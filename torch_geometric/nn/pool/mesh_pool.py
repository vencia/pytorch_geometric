...
def mesh_pool(data, count, aggr='add'):
    for _ in range(count):
        ...
        if len(x_neighbors) == 0:  # isolated vertex
            data.x[y_min] = data.x[x_min] + data.x[x_min]  # some high value , TODO: change
            continue
        ...
        if aggr == 'max':
            new_node_attr = torch.max(data.x[x_min], data.x[y_min])  # TODO: doesn't work because of inplace operation?
        elif aggr == 'mean':
            new_node_attr = (data.x[x_min] + data.x[y_min]) / 2
        else:
            assert aggr == 'add'
            new_node_attr = data.x[x_min] + data.x[y_min]
        data.x[x_min] = new_node_attr
        data.x[y_min] = data.x[x_min] + data.x[
            x_min]  # some high value, TODO: change, because then wrong global pooling etc
